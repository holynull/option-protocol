    ...
    function getPrice(address asset) public view returns (uint256 price) {
        if (asset == address(0)) {
            ...
            if (isCToken[asset]) {
                ...
                uint256 exchangeRate = cToken.exchangeRateStored(); // wei
                if (isCETH(asset)) {
                    uint256 numerator = 10**46;
                    price = numerator.div(exchangeRate);
                }
                address underlyingAddress = cToken.underlying();
                // cTokenPriceInETH = underlying price in ETH * (cToken : underlying exchange rate)
                price = getPriceUnderlying(underlyingAddress).mul(exchangeRate);
            } else if (assetToCTokens[asset] != address(0)) {
                //2. Underlying Tokens that Compound lists
                price = getPriceUnderlying(asset);
            }
        }
    }

    function getPriceUnderlying(address asset)
        internal
        view
        returns (uint256 EthToAssetPrice)
    {
        EthToAssetPrice = PriceOracle.getUnderlyingPrice(
